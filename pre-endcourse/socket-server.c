...
int server(int client_socket)
{
    while (1)
    {
        ...
        if (read(client_socket, &length, sizeof(length)) == 0)
            return 0; // Cap phat vung dem cho text
...
int main(int argc, char *const argv[])
{
    ...
    bind(socket_fd, &name, SUN_LEN(&name)); /* Lang nghe ket noi*/
    listen(socket_fd, 5);
    ...
    do
    {
        ...
        /* Chap nhan ket noi */
        client_socket_fd = accept(socket_fd, &client_name, &client_name_len); /* Dieu khien ket noi */
